...
void display()
{
    ...
    //使所有物体都饶相同的地方旋转==使相机旋转。



    GLint list[]={4,5};
    glListBase(0);//设置读取数组时，数组中每个值的偏移量，默认为0，其实最好不用设置。
    glCallLists(2,GL_INT,list);//调用多少个列表，列表数组的类型，列表数组。

    //glCallList(AxisLineRGB);
    //glCallList(SolidTeapot);
    glDeleteLists(SolidTeapot,1);//只绘制1次，就删除SolidTeapot这'1'个列表。
...
void buildMyobject()
{
    //glNewList( glGenLists(1) ); 这样用也是可以的。这个函数用于申请几个新的显示列表ID，会返回第一个可用的ID。
    glNewList(AxisLineRGB,GL_COMPILE);
        glPushAttrib(GL_CURRENT_BIT);//绘制开始，先保存
