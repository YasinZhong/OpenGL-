...
为了启动深度缓冲区，必须先启动它，即glEnable(GL_DEPTH_TEST)。
每次绘制场景之前，需要先清除深度缓冲区，即glClear(GL_DEPTH_BUFFER_BIT)，然后以任意次序绘制场景中的物体。
详见 init()、myobject()
*/
...
void myobject()
{
    ...
    glColor3f(0.1,0.3,0.4);//记得恢复绘制颜色
    glTranslatef(0,0,-4);//在原位再向某方向移动
    glutWireTeapot(3);
