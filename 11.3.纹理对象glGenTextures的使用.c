...
关键代码位于
    全局变量
    函数：void colorcube(void)
    main()
    myobject()

    glGenTextures(3,name);
    glBindTexture(GL_TEXTURE_2D,name[0]);//设置纹理形式的时候、切换贴图的时候使用
    glIsTexture(name[0]);//检查是否占用
    glDeleteTextures(n,name);//按照顺序删除对象
    //右面与前面将会显示与众不同的贴图
...
void colorcube(void)
{
	...
	glBindTexture(GL_TEXTURE_2D,name[1]);//右面与前面将会显示与众不同的贴图
	glNormal3f(1,0,0);//右面法向
	polygon(1,2,6,5);

	glBindTexture(GL_TEXTURE_2D,name[2]);//右面与前面将会显示与众不同的贴图
...
int main(int argc, char **argv)
{
    ...
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_PRIORITY,0.99);//此纹理在内存不足时第99名被删除
    ...
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_PRIORITY,0.1);//此纹理在内存不足时第1名被删除
    ...
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_PRIORITY,0.11);//此纹理在内存不足时第11名被删除
